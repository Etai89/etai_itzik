...
// 'AIzaSyCPSJxU8dcbrnilAzjzZbNWG73sf3NWqeM'
        ...
        const apiEndpoint = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${token}`
    ...
    // Function to clean and format the response into a proper recipe
    function cleanResponse(response) {
        // Remove unwanted symbols, extra asterisks, and tags
        response = response.replace(/\*\*|\*|\*\*|<\/?[^>]+(>|$)/g, '')  // Remove all markdown and HTML tags

        // Format ingredients
        response = response.replace(/Ingredients:/i, '<h3>Ingredients</h3><ul>')
                            .replace(/(?:\s*\*\s*(.+?)\s*\*)/g, '<li>$1</li>') // Convert ingredients list to <ul><li> format
                            .replace(/<\/ul>/, '</ul>') // Close the list at the end

        // Format instructions
        response = response.replace(/Instructions:/i, '<h3>Instructions</h3><ol>') 
                            .replace(/(?:\d+\.\s*(.+?))(\s*(?=\d+\.|$))/g, '<li>$1</li>') // Convert instructions to <ol><li> format
                            .replace(/<\/ol>/, '</ol>'); // Close the ordered list
